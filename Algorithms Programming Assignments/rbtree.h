...
public:
    ...
    size_t diameter() const {
        ...
        if (root_->left) dleft = diameter(root_->left);
        if (root_->right) dright = diameter(root_->right);
        int side_max = std::max(dleft, dright);
        return (size_t)std::max(diameter(root_),side_max);
    }
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
        while (z->parent && z->parent->color==RED) {
            if ( (z->parent) && (z->parent->parent) && z->parent == z->parent->parent->left ) {
                Node<K,V> * y; 
                if (z->parent->parent) y = z->parent->parent->right;
                else y = nullptr;
                if ( y && y->color == RED ){
                ...
                Node<K,V> * y;
                if (z->parent->parent) y = z->parent->parent->left;
                else y = nullptr;
                if ( y && y->color == RED ){
                    if (z->parent) z->parent->color = BLACK;
                    y->color = BLACK;
    ...
    int height(Node<K, V> *node) const {
        ...
        if (node->left) height1 = height(node->left)+1;
        if (node->right) height2 = height(node->right)+1;
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        if (!node) return 0;
        if (node->right && node->left) return 1+internal_node_count(node->right)+internal_node_count(node->left);
        if (node->right) return 1 + internal_node_count(node->right);
        if (node->left) return 1 + internal_node_count(node->left);
        return 0;
    }
    ...
    int diameter(Node<K, V> *node) const {
        int h1 =0, h2=0;
        if (node->left) h1 = height(node->left)+1;
        if (node->right) h2 = height(node->right)+1;
        return 1 + h1 + h2;
    }
